...
public class TV {
	...
	public int ch;
	public int vol;
	public boolean onOff;	// false가 초기값
	
	// 다른 생성자 메서드가 없으면 부품만들때(객체생성) 자동으로 생성딤
...
//	}
	...
	// 객체생성할 때마다 꼭 실행시키고싶은 내용이 있으면 기본 생성자를 눈에 보이게
	// 다시 만들어라 (명시적으로)
	public TV() {
		// source -> generate constructors from super~
	}
...
//		// 무더기 영역(heap영역)에 저장된 멤버변수에 값을 넣어주는 역할
//		this.ch = ch;
//		this.vol =  vol;
//		this.onOff = onOff;
//	}
	
	// 파라메터 있는 생성자가 있을 때는 기본 생성자는 자동생성되지 않는다
	// 자동완성 : source -> generate constructors using fields~
