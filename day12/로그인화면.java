...
public class 로그인화면 {
	...
	public static void main(String[] args) {
		...
		// 1)버튼에 액션기능을 add
		// 2)클릭했을 때 "어떤 부품이" 처리를 담당할지 new로 만들자
		// 3)클릭했을 때 "어떻게 처리할지"를 쓰자
		reset.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				// reset을 눌렀을때 idText, pwText에 공백을 넣어라
				idText.setText("");
				pwText.setText("");
			}
		});		// reset add end
		...
		login.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (id2.equals("root") && pw2.equals("1234")) {
					...
					f.setVisible(false);	// 화면 안보이게 하기 (페이지 숨기기)
				} else {
